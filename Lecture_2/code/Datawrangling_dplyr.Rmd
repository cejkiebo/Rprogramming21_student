...
```{r}
# Here your code
ggplot(df1, aes(x = year, y = lifeExp, by=country)) +
...
ggplot(df2, aes(x = year,
                y = lifeExp,
                group = country)) +
  geom_line(aes(colour = Netherlands, alpha = Netherlands)) +
...
# Here your code
ggplot() +
  geom_point(data = df4, aes(x = Mean_LE,
                             y = reorder(continent, Mean_GDP),
                             size = Mean_GDP),
             colour = "blue") +
  geom_segment(data = df4, aes(x = 0,
                               xend = Mean_LE,
                               y = continent,
                               yend = continent),
               colour = "blue") +
  theme_bw() +
  labs(caption = "source: gapminder dataset",
       x = "Weighted mean of life expectancy",
       y = "",
       size = "Weighted mean of GDP per capita")
```
...
```{r}
# Here your code
withlabel <- c("Guinea-Bissau", "Swaziland", "India", "Pakistan", "China", "Mexico", "Chile", "United States", "Netherlands", "United States", "Spain", "Japan")


dfLabeled <- gapminder %>%
  filter(year == 2007 & country %in% withlabel)

dfUnlabeled <- gapminder %>%
  filter(year == 2007 & !country %in% withlabel)

ggplot(data=dfLabeled, aes(x = log(gdpPercap),
                      y = lifeExp,
                      colour = continent,
                      size = pop,
                      label = country)) +
  geom_point(data = dfUnlabeled, aes(x = log(gdpPercap),
                                     y = lifeExp,
                                     colour = continent,
                                     size = pop)) +
  geom_point() +
