...
```{r}
Q1_xlsx <- read_xlsx("../Sourcedata/dataQ1.xlsx") %>%
  filter(Group == "Group1")

ggplot(Q1_xlsx, aes(x = Variable_1, y = Variable_2)) +
  geom_point()
```
Answer B: Heart
...
```
Mean life expectancy of Oceania is 81.
...
tibble(x = rnorm(1000, mean = 100, sd = 10)) %>%
  mutate(y = (sin(x^2) + 33 * (x^4))) %>%
  summarise(mean_y = mean(y))
```
mean of y is ~~93.71417~~ 3518271089.
