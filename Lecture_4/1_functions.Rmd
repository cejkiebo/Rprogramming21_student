...
`sum()` requires a vector, and `na.rm` is optional but set to `FALSE` by default.
...
> log(100) YES
> log(base = exp(1)) NO
> log(exp(1)^3, exp(1)) YES
> log(base = 10, x = 100) YES
...
df_2018 <- readRDS("./sourcedata/clothing_exports_aggregated.rds") %>%
  filter(year == 2018)
length(unique(df_2018$reporter_iso))
```
...
# your code here
for(cc in countries) {
  print(plot_top_five_receiving_countries(dataset = df_2018, country_code = cc))
}
```
